...
export class DataModelOptimizationAgent implements AutonomousWorker {
  ...
  async execute(action: AutonomousAction, context: HarnessContext): Promise<AutonomousWorkerResult> {
    const prompt = `You are the Data/Model Optimization Agent for a supervised autonomous harness run.
Your responsibility is dataset analysis, model architecture selection, running experiments, tracking metrics against baseline, and hyperparameter tuning.
Ensure all experiments are fully tracked and no data leakage occurs.
