...
export class RefactorAgent implements AutonomousWorker {
  ...
  async execute(action: AutonomousAction, context: HarnessContext): Promise<AutonomousWorkerResult> {
    const prompt = `You are the Refactor Agent for a supervised autonomous harness run.
Your responsibility is progressive module migration, keeping API compatibility, and maintaining architectural direction.
Execute exactly one bounded refactoring action. Do not make unrelated changes.
